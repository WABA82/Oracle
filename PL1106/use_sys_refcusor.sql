...
--입력된 매니저 번호가 emp테이블에 존재하는 매니저 번호 일때만 검색을 수행한다.
--(7839 7782 7698 7902 7566 7788)

set verify off
set serveroutput on
	accept mgr prompt '매니저 번호 : ';
declare
	i_mgr number;
	
	--1.커서열기sys_refcursor 선언 (데이터형)
...
begin
	...
	for temp_mgr in (select distinct mgr from  emp where mgr=&mgr) loop
		i_mgr := temp_mgr.mgr;    
    end loop;
    
    if i_mgr is not null then
    	dbms_output.put_line('매니저 존재');
	--2.커서열기 : select 쿼리문 작성
	open cur_mgr for
