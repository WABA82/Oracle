...
--class4 학생들의 번호, 이름, 오라클점수를 처리하는 PL/SQL 만들기
declare
	--학생의 정보를 저장할 레코드 선언
	type stu_record is record
		(num number(4), name varchar2(30), oracle_score number(3));
	--학생들의 정보를 저장할 테이블 선언
	type stu_table is table of stu_record
		index by binary_integer;
	--테이블을 사용해서 학생들의 정보를 처리할 변수 선언
...
--		dbms_output.put_line('번호 : ' || class4_tab(i).num ||'		'|| class4_tab(i).name||'		'|| class4_tab(i).oracle_score||'		'||);
--		
